...
export default function CursorSmoke() {
  ...
  useEffect(() => {
    ...
    // Modified display shader to output alpha based on color intensity so it blends on light backgrounds
    let displayShader = compileShader(
      gl.FRAGMENT_SHADER,
      "precision highp float; precision mediump sampler2D; varying vec2 vUv; uniform sampler2D uTexture; void main () {     vec3 c = texture2D(uTexture, vUv).rgb; float a = max(c.r, max(c.g, c.b)); gl_FragColor = vec4(c, a * 0.8); }"
    );
    ...
    function update() {
      ...
      gl.clear(gl.COLOR_BUFFER_BIT); // Clear transparency

      // Enable blending so smoke blends smoothly
      gl.enable(gl.BLEND);
    ...
    let count = 0;
    // Elegant warm light colors: Gold, Silver, Bronze, light yellow, soft orange
    let lightColors = [
      [1.0, 0.8, 0.2], // Gold
      [0.8, 0.8, 0.85], // Silver
      [0.9, 0.5, 0.2], // Bronze
    ...
    let colorArr = lightColors[Math.floor(Math.random() * lightColors.length)];

    const handleMouseMove = (e: MouseEvent) => {
      count++;
      if (count > 25) {
        colorArr = lightColors[Math.floor(Math.random() * lightColors.length)];
        count = 0;
      }
    ...
    // Attach to window so it tracks over the whole screen even though canvas is pointer-events-none
